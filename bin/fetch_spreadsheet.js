...
if (!nconf.get('USE_JWT')) {
  ...
  authClient = new JWTClient(
    nconf.get('GOOGLE_SERVICE_EMAIL_ACCOUNT'),
    null,
    nconf.get('GOOGLE_SERVICE_CREDENTIAL'),
...
function buildDataJSON(data) {
  ...
  fs.writeFileSync('./app/assets/data/agenda.json', JSON.stringify(payload.FUTURE));
  fs.writeFileSync('./app/assets/data/tracker.json', JSON.stringify(payload.PAST));
}

primeToken(authClient, function() {
  drive.files.get({
    auth: authClient,
